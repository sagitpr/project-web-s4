...
target = 'django_backend/templates/auth/login/index.html'
os.makedirs(os.path.dirname(target), exist_ok=True)
with open(target, 'w', encoding='utf-8') as f:
    f.write(content)
print('FILE CREATED:', target)
...
h = render_to_string('auth/login/index.html', {})
print()
print('=== RENDERING VERIFICATION ===')
checks = {
    'CSS login style': 'css/pages/auth/login/style.css' in h,
    'CSS responsive': 'css/responsive.css' in h,
    'JS auth': 'js/auth.js' in h,
    'JS api': 'js/api.js' in h,
    'JS login script': 'js/pages/auth/login/script.js' in h,
    'Logo image': "images/Warungio L.png" in h,
}
for name, ok in checks.items():
    print(f'  {name}: {"OK" if ok else "FAIL"}')
print(f'  Artifacts (%}): {"CLEAN" if "%}" not in h else "FOUND"}')
