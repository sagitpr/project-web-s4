  ...
  function reverseGeocode(lat, lng) {
    ...
    geocoder.geocode({ location: { lat, lng } }, (results, status) => {
      if (status === 'OK' && results[0]) {
        ...
        // Parse address_components to fill province, city, district, postal_code
        const components = results[0].address_components || [];
        let province = '', city = '', district = '', postalCode = '';
        components.forEach(function(c) {
          ...
          if (types.indexOf('administrative_area_level_3') !== -1 || types.indexOf('sublocality_level_1') !== -1 || types.indexOf('locality') !== -1) {
        ...
        if (form) {
          ...
          if (form.elements.postalCode && postalCode) {
            ...
            form.elements.postalCode.dispatchEvent(new Event('input'));
          }
        }
      }
    });
