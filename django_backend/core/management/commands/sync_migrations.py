"""
Full Django management command: sync_migrations (Permanent Solution).

Problem Statement:
    Database MariaDB sudah memiliki seluruh tabel bisnis (users, orders, products, stores, dll.)
    tetapi tabel django_migrations tidak sinkron dengan schema migration files.
    token_blacklist belum pernah dimigrasikan sehingga login gagal (HTTP 500).

Strategy (Idempotent — aman dijalankan berkali-kali):
  ...
  4. Jalankan `migrate` untuk migration yang memang belum ada (termasuk token_blacklist)
  5. Idempotent: aman dijalankan berkali-kali

Usage:
    python manage.py sync_migrations               # Full sync (standard)
    python manage.py sync_migrations --dry-run      # Preview only — no DB changes
    python manage.py sync_migrations --fake-all     # Force-fake ALL remaining (emergency)
    python manage.py sync_migrations --app orders   # Sync only one app
    python manage.py sync_migrations --no-backup    # Skip django_migrations backup
"""
...
# ──────────────────────────────────────────────────────────────────────────
# TYPE NORMALIZATION for per-field type comparison
# ──────────────────────────────────────────────────────────────────────────
_DJANGO_TYPE_GROUPS = {
    'IntegerGroup': [
        'ForeignKey', 'OneToOneField', 'AutoField', 'BigAutoField',
...
def _get_column_name(app_label, model_name, field_name):
    ...
    try:
        ...
        return f"{field_name}_id" if not field_name.endswith('_id') else field_name


class Command(BaseCommand):
    help = (
        "Sync Django migrations with pre-existing database. "
        "Backup → per-op validation → fake existing → migrate missing (incl. token_blacklist). "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--fake-all', action='store_true',
                            help='Force-fake ALL remaining migrations (emergency only)')
        ...
        parser.add_argument('--verbose', action='store_true',
                            help='Show detailed per-migration breakdown')
    ...
    def handle(self, *args, **options):
        ...
        # ── Step 7: Report ──
        self._print_report(to_fake, to_run)
        ...
        # ── Step 9: Run migrate for genuinely missing migrations ──
        if not self.dry_run:
            if to_run:
                self.stdout.write(self.style.WARNING(
    ...
    def _topo_sort(self, graph):
        """Topological sort of migration graph."""
        visited = set()
        result = []
    ...
    # ──────────────────────────────────────────────────────────────────────
    # PER-MIGRATION REFLECTION CHECK
    # ──────────────────────────────────────────────────────────────────────
    ...
    def _check_single_op(self, op, app_label, existing_tables,
                          table_columns, table_constraints, table_descriptions):
        """Periksa satu operasi migration terhadap database yang ada."""
        ...
        if isinstance(op, CreateModel):
            ...
            for fname, finst in op.fields:
                if finst.__class__.__name__ in ('GenericRelation',):
                    continue
        ...
        if isinstance(op, RenameField):
            ...
            if db_table not in existing_tables:
                return False
            db_cols = table_columns.get(db_table, set())
            return (col(op.model_name, op.old_name) not in db_cols
                    and col(op.model_name, op.new_name) in db_cols)
        ...
        if isinstance(op, AddIndex):
            ...
            if op.index.name and op.index.name in constraints:
                if constraints[op.index.name].get('index'):
                    return True
        ...
        # ── Non-schema ops (AlterModelOptions, RunPython, RunSQL, etc.) ──
        return True
    ...
    def _print_report(self, to_fake, to_run):
        ...
        if to_run:
            ...
            for app, name in to_run[:10]:
                self.stdout.write(f'    {app}.{name}')
            if len(to_run) > 10:
                self.stdout.write(f'    ... and {len(to_run) - 10} more')

        # Special highlight for token_blacklist
        token_blacklist_pending = [n for n in to_run
