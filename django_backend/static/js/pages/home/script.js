    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        data.results.forEach(p => {
          ...
          const img = p.image || '/static/images/vega-fresh.png';
          ...
          productGrid.innerHTML += `
            <div class="product-card" data-id="${p.id}">
              ...
              <div class="card-body">
                ...
                <button class="btn-buy" onclick="window.location.href='/auth/login/'">Beli Sekarang</button>
              </div>
            </div>`;
        });
      }
        ...
        data.results.forEach(s => {
          storeGrid.innerHTML += `
            <div class="store-card" data-id="${s.id}">
              <img src="${s.logo || '/static/images/store-icon-T.png'}" alt="${s.store_name}" />
              <h4>${s.store_name}</h4>
              <p>${s.city || ''}</p>
