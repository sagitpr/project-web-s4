      ...
      if (data.results && data.results.length > 0) {
        ...
        document.querySelectorAll('.btn-cart').forEach(btn => {
          btn.addEventListener('click', async (e) => {
            ...
            try {
              await WarungioAPI.addToCart(productId, 1, storeId);
        ...
        data.results.forEach(p => {
          container.innerHTML += `
            <div class="product-card mini" data-id="${p.id}">
              <img src="${p.image || WarungioAssets.img('vega-fresh.png')}" alt="${p.name}" />
              <h4>${p.name}</h4>
      ...
      orders.forEach(function (o) {
        ...
        card.innerHTML =
          ...
          '<div class="order-card-footer">' +
            ...
            '<a href="../order-detail/index.html?id=' + o.id + '" class="order-card-link">' +
              'Lihat Detail' +
